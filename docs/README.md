## 서비스 소개
project: 대파마켓 

대파마켓은 기존 중고거래 플랫폼에서 발생하는 가격 결정과 협상 과정의 비효율성을 보완한 중고거래 플랫폼이다.
판매자가 설정한 가격 범위 내에서 시스템 알고리즘으로 거래가 성사될 수 있는 가격으로 변동시켜주는 중고거래 서비스.

대파마켓은 판매자가 상품을 등록할 때, 가격 정책을 미리 설정한다.
- 판매 시작 가격
- 최소 판매 가격

판매자와 구매자의 회전율이 중요해. 그래서 둘 다의 유입이 많아야 하는데 판매자에게는 빨리 팔리고, 가격 고민을 줄이고, 헐값 판매를 막는다, 네고의 스트레스를 없앤다 라는 이미지를 줘야 해. 

상품 상세에는 페이지에 상품 가격 변동을 그래프로 보여주고 가격이 변동된 이유도 보여줘야 해.



## 자동 가격 조정 시스템

구현할 때 아래 정책을 반드시 지켜줘.

1. 상품은 등록 시 판매자가 다음 가격을 입력한다. 
- initialPrice: 최초 등록 가격
- minimumPrice: 판매자가 허용하는 최저 판매 가격

자동 가격 조정은 모든 상품에 필수 적용된다.
사용자가 자동 가격 조정 여부를 ON/OFF 할 수 있는 옵션은 만들지 않는다.
자동 알고리즘으로 최초 등록 가격보다 가격이 높아져서는 안되고 최저 판매가격보다 낮아져서도 안된다


2. 가격 조정 정책
등록 또는 마지막 가격 조정 기준 24시간 이내 관심 수로 판단한다.
신규 관심 0명 → 3% 인하
신규 관심 1~2명 → 유지
신규 관심 3명 이상 → 1% 상승
신규 관심 5명 이상 → 3% 상승
신규 관심 10명 이상 → 5% 상승


3. 실행 순서
  1. 판매중 상품 조회
  2. 등록 또는 마지막 평가 이후 24시간이 지났는지 확인
  3. 24시간 이내 신규 관심수 확인
  4. 가격 조정 정책에 따라 가격 변경

홈 화면의 `가격 재책정` 버튼은 목업 시연용이다. 로그인이나 관리자 권한을
요구하지 않으며, 마지막 평가 후 경과 시간을 확인하지 않고 판매중 상품을 모두
즉시 한 번씩 평가한다. 평가 직전에 데모 계정의 관심 데이터를 상품별 0~10개로
무작위 생성하므로 가격 상승, 유지, 인하 결과를 함께 시연할 수 있다. 일반 회원의
관심 데이터는 변경하지 않는다. 자동 가격 조정 작업은 기존 시간 조건을 그대로
사용한다.



## 로그인/회원가입
이메일과 비밀번호를 입력하여 서비스 이용 할 수 있다. 

입력 정보: 닉네임, 이메일, 비밀번호, 주소
중점사항: 필수 입력값 검증, 이메일 중복 확인, 비밀번호 유효성 검사, 회원가입 성공/실패 처리
주소는 Kakao 우편번호 검색에서 선택하며 상세주소는 저장하지 않는다.
회원 주소는 시·도, 시·군·구, 읍·면·동을 조합한 거래 지역 문자열로 저장한다.

## 메인 페이지
관심 수에 따른 자동 가격 조정 시스템을 직관적으로 이해할 수 있어야 한다.

- 인기상품(사용자가 관심가지는 상품 view, click)
- 최근 등록상품(판매중 상품을 등록일이 최신인 순서로 표시)

서비스 컨셉을 이해하고, 상품 탐색까지 자연스러운 이동
현재 가격 변동 강조

## 상품 등록 페이지 
판매자가 상품과 가격 정책을 등록한다. 

상품정보: 상품이미지, 상품명, 카테고리, 상품 설명, 거래 지역
가격 정책: 판매 시작 가격, 최소 판매 가격

## 상품 상세 페이지 
구매자가 상품 정보를 확인하고 가격 변동 상황을 파악한 후 구매할 수 있다.

정보: 상품이미지, 상품명, 카테고리, 상품 설명, 거래 지역, 판매 시작 가격, 최소 판매 가격, 상품 상태

## 상품 검색 페이지
상품 목록 검색은 원하는 조건으로 상품을 검색할 수 있는 기능이다.

왼편에 카테고리를 선택할 수 있게 하고 지역과, 가격대도 조건으로 지정할 수 있다.
정렬은 최신순, 등록일 순, 낮은 가격순, 높은 가격순, 관심 많은 순, 많이 썰린 순(최초 가격 대비 할인율이 큰 상품), 시장가 대비 저렴한 순(유사 상품 시세보다 현재 가격이 낮은 상품)으로 할 수 있게 한다.


## 마이페이지
마이페이지에는 내 활동, 판매관리, 구매내역, 관심상품, 설정과 같은 하위 기능이 있다.

내 활동 : 에는 판매하고 있는 상품 개요, 구매한 상품 개요, 관심상품 개요를 간단히 보여준다
판매 관리 : 판매하고 있는 상품의 리스트를 보여준다.
구매내역 : 구매를 한 상품의 리스트를 보여준다.
설정 : 하위 메뉴로 프로필 수정(닉네임, 거래지역, 주소), 거래 설정(기본 거래 지역, 채팅 가능 시간), 계정 설정(비밀번호 변경, 로그아웃, 회원탈퇴)

 
// 프롬프트는 여기에 작성


## DB컬렉션(몽고DB)

[users:회원]
_id : 고유 키값
email : 아이디로 사용할 것이고 고유의 값
nickname : 닉네임
address : Kakao 주소 검색으로 선택한 거래 지역(시·도, 시·군·구, 읍·면·동, 상세주소 제외)
emailVerified : 이메일 확인 여부
image : 프로필 이미지 주소
createdAt : 가입일
updatedAt : 회원정보 수정일
role : 일반회원/관리자

[account:Better Auth 인증 계정]
_id : 고유 키값
accountId : 인증 제공자에서 사용하는 계정 키(Better Auth 사용자 키)
providerId : 인증 제공자(이메일 로그인은 credential)
userId : users 컬렉션을 참조하는 Better Auth ObjectId
password : Better Auth 형식의 비밀번호 해쉬값
createdAt : 계정 생성일
updatedAt : 계정 수정일

개발용 seed 사용자는 `admin@daepa.test`, `minsu@daepa.test`, `jiyun@daepa.test`,
`seojun@daepa.test`, `yuna@daepa.test`이며 공통 비밀번호는 `Demo1234!`이다.
가격 재책정 시연을 위해 `interest01@daepa.test`부터 `interest06@daepa.test`까지의
관심 전용 계정도 같은 비밀번호로 생성한다.
`npm run seed`는 업무 데이터와 Better Auth의 account, session, verification 데이터를
초기화하므로 운영 환경에서는 실행할 수 없다.
개발용 seed에는 여러 카테고리의 상품 30개와 상품별 이미지 5장이 포함된다.
기존 개발 데이터를 유지하면서 카테고리만 추가하거나 갱신할 때는
`npm run categories:seed`를 사용한다.



[categories:카테고리]
_id : 고유 키값
name : 상품 등록과 검색에서 표시할 카테고리 이름
sortOrder : 선택 목록 표시 순서

[products:상품]
_id : 고유 키값
images : 상품이미지(최대 5개, 파일당 5MB 이하)
name : 상품명
category : categories 컬렉션에 등록된 카테고리 이름
info : 상품 설명
initialPrice : 판매 시작 가격
minimum : 최저 판매 가격
region : 거래 지역
status : 상품 상태 (최상, 상, 중, 하)
createdAt : 상품등록 날짜


[watchlists:관심목록]
_id : 고유 키값
productId : 상품 키값
userId : 회원 키값
createdAt : 관심목록 추가한 날짜

[transactions:거래]
_id : 고유 키값
buyerId : 구매자
sellerId : 판매자
productId : 상품 키값
price : 거래 성사 가격
createdAt : 거래 성사 날짜

[priceChange:가격변동]
_id : 고유 키값
productId : 상품 키값
changedAt : 변경된 날짜
previousPrice : 이전 가격
newPrice : 변경된 가격
seq : 변경된 횟수
reason : 변경 이유
status : 가격이 올랐는지 내렸는지 KEEP인지(UP/DOWN/KEEP)


